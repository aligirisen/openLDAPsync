    ...
    with open(filename_ad, 'r') as file:
        for line_number, line in enumerate(file, start=1):
            if line.startswith('#') or line.startswith(' '):
                if dn != " " and sAMAccountName != " " and cn != " " :
                    if input_dn and spec_search:
                        ...
                        if search_result:
                            ...
                            user_membership = conn.search(ldap_group_dn, search_filter=search_filter_group)
                            if user_membership:
                                for group in ldap_existing_groups:
                                    group_dn = f"cn={group},{ldap_group_dn}"
                                    group_existing = conn.compare(group_dn, "member", dn)
                            ...
                            if conn.search(f'{group_name},dc=alidap,dc=com', '(objectClass=*)'):
                                if exist_uid:
                                ...
                                result = conn.add(dn, attributes=new_attributes)
                                #if not result:
                                #    print(f"CHECK THE SYNC GROUP PREFERENCES MAY GROUP NOT EXISTING Failed to add user: {conn.result}")
                    dn,cn,sn,givenName,email,phoneNumber,sAMAccountName,memberOf = " ", " ", " ", " ", " ", " ", " ", []
                ...
                formatted_components.append('ou=Groups')
...
command_ad = f'ldapsearch -x -D {ad_username} -w {ad_password} -H ldap://{ad_server}:{ad_port} -b {ad_base_dn} "(&(objectClass=person)(objectCategory=person)(!(sAMAccountName=krbtgt))(!(sAMAccountName=Administrator))(!(sAMAccountName=Guest)) )" -s sub -E pr=1000/noprompt  cn sn telephoneNumber mail givenName sAMAccountName memberOf > ad_users.ldif'
